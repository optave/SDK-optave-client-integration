...
optaveClient.on('message', (payload) => {
    ...
    if (message.headers?.action === 'interaction') {
        console.log('✅ interaction response received:', message.payload);
    }
});
...
async function sendInteractionRequest() {
    try {
        ...
        const response = await optaveClient.interaction({
          "headers": {
        ...
        console.log('✅ interaction request sent successfully:', response);
    } catch (error) {
