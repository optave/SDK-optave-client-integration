...
optaveClient.on('message', (payload) => {
    ...
    if (message.headers?.action === 'recommend') {
        console.log('✅ recommend response received:', message.payload);
    }
});
...
async function sendRecommendRequest() {
    try {
        console.log('🚀 Sending recommend request...');

        const response = await optaveClient.recommend({
          "headers": {
        ...
        console.log('✅ recommend request sent successfully:', response);
    } catch (error) {
        console.error('❌ Failed to send recommend request:', error);
    }
}
